...
--A simple version with a statically sized flame.
------------------------------------------------------------------------------------
...
function love.update(dt)
    ...
    if goingUp == true then --we could just say if goingUp then
        ...
        --Is there enough space to show the flame?
        if rocketCornerY < love.graphics.getHeight() - flameHeight * 1.5 then
          drawFlame = true
        end --end can we show the flame loop 
        ...
        if rocketCornerY >=love.graphics.getHeight() - (startSpeed/2) then 
            goingUp = true
            print ("reversing"..rocketCornerY)
